...
class cosyneBreakoutRam(cosyne):
    def __init__(self, subPopLen):
        ...
        self.outWInd = self.dense1BInd + 256*self.nOut
        self.outBInd = self.outWInd + self.nOut
    ...
    def trainOneEpoch(self, evaluator):
        ...
        for net in range(self.subPopLen):
            ...
            outW = self.subpop_nodes[net, self.dense1BInd:self.outWInd].reshape((256, self.nOut))
            outB = self.subpop_nodes[net, self.outWInd:self.outBInd]

            def get_net_out(input):
                denseAct = np.maximum(0, (np.dot(input, dense1W) + dense1B))
                return np.dot(denseAct, outW) + outB

            net_learner = evolvedLearner(get_net_out)

            evaluator.set_legal_actions(net_learner)
            fitness = evaluator.evaluate(net_learner)
            print(net, fitness)
            fitnessNodes[net] += -1*fitness
