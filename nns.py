...
class CNN:
    def __init__(self, inpShape, outputNum, clip, stride=(4,2)):
        import pickle
        with open('crbmparms.pkl', 'rb') as inFile:
            w,b,w2,b2 = pickle.load(inFile)
            w = np.swapaxes(w,0,1).reshape(-1, 8, 8, 3)
            w = np.swapaxes(w,1,3)
            w = np.swapaxes(w,2,3)
            w = np.asarray(w, dtype=theano.config.floatX)
            b = np.asarray(b, dtype=theano.config.floatX)
            w2 = np.swapaxes(w2,0,1).reshape(-1, 4, 4, 16)
            w2 = np.swapaxes(w2,1,3)
            w2 = np.swapaxes(w2,2,3)
            w2 = np.asarray(w2, dtype=theano.config.floatX)
            b2 = np.asarray(b2, dtype=theano.config.floatX)
        self.l_in = lasagne.layers.InputLayer(inpShape)
        # self.l_hid1 = lasagne.layers.Conv2DLayer(self.l_in, 16, (8, 8), W=w, stride=4, untie_biases=True)
        ...
        self.l_pool1 = lasagne.layers.MaxPool2DLayer(self.l_hid1, 2)
        # self.l_hid2 = lasagne.layers.Conv2DLayer(self.l_pool1, 32, (4, 4), W=w2, stride=2, untie_biases=True)
