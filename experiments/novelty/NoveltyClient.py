...
class AsyncNStepNoveltyLearner(AsyncTargetLearner):
    def __init__(self, num_actions, initial_cnn_values, pipe,
                 skip_frame=4, phi_length=4, async_update_step=5, discount=0.95, target_update_frames=40000,
                 additional_args=None):
        cnn_partial = partial(AsyncTargetCNNNstep, (None, phi_length, 84, 84), num_actions, async_update_step)
        ...
        if additional_args is None:
            raise AttributeError("additional args (the novelty dictionary) must be passed by the host")
        self.novel_frames = additional_args
